...
demo, chat_history, show_img, txt, submit_button, uploaded_pdf, uploaded_timeseries = create_demo()
...
with demo:
    ...
    submit_button.click(pdf_chatbot.add_text, inputs=[chat_history, txt], outputs=[chat_history], queue=False).\
        success(pdf_chatbot.generate_response, inputs=[chat_history, txt, uploaded_pdf, uploaded_timeseries], outputs=[chat_history, txt]).\
